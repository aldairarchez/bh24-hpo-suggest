...
def script(hpo_filename):
    ...
    # Print out hpo terms along with terms that are 1 degree away
    for term in hpo:
        term_ids = [term, *term.alts]
        adjacent_terms = set()
        for adj in [*term.parents, *term.children]:
            adjacent_terms.add(adj.id)
            adjacent_terms.update(adj.alts)
        print('{}\t{}'.format(comma_delimit(term_ids), comma_delimit(adjacent_terms)))
